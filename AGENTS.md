...
```
/
+-- devops-learn/          # Main Next.js app
|   +-- src/
|   |   +-- app/
|   |   |   +-- page.tsx              # Homepage: hero, search, category filters, topic grid
|   |   |   +-- layout.tsx            # Root layout (system fonts, ThemeProvider)
|   |   |   +-- globals.css           # Tailwind v4 config, CSS vars, mesh-bg, glass
|   |   |   +-- topic/[slug]/
|   |   |       +-- page.tsx          # Server component: generateStaticParams + wrapper
|   |   |       +-- topic-content.tsx # Client component: Q&A accordion, search, progress
|   |   +-- components/
|   |   |   +-- navbar.tsx            # Sticky nav: logo, progress count, theme toggle, GitHub link
|   |   |   +-- topic-card.tsx        # Topic card with color accent, progress bar
...
|   +-- scripts/
|   |   +-- convert-data.mjs          # Parses Markdown topics/ -> topics.json
|   +-- next.config.ts                # output: "export", BASE_PATH from env
|   +-- public/.nojekyll
|   +-- package.json
+-- topics/                          # Original Markdown exercises (source data)
+-- certificates/                    # Certification study guides
+-- coding/                          # Coding exercises
+-- exercises/                       # Shell/Docker exercises
+-- images/                          # Repo images
+-- .github/workflows/
|   +-- deploy-pages.yml             # Auto-deploy to GitHub Pages on push to master
